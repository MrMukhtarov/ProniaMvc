...
using ProniaMvc.ViewModels.AuthVMs;
...
public class AuthController : Controller
{
    ...
    public AuthController(UserManager<AppUser> userManager, SignInManager<AppUser> signInManager,
        RoleManager<IdentityRole> roleManager, IEmailService emailService, ProniaDbContext context)
    {
    ...
    public async Task<IActionResult> ConfirmationEmailSent(string username)
    {
        ...
        var utoken = await _context.UserTokens.SingleOrDefaultAsync(ut => user.Id == ut.AppUserId && ut.Key == "EmailConfirmation");
        if (utoken?.SendDate.AddHours(24) > DateTime.UtcNow)
        {
            return Content("", "Mail already sent. if you dont see in your inbox. Please check spams. You can send " +
                "Confirmation mail after " + utoken.SendDate.AddHours(24));
        }
    ...
    public async Task<IActionResult> Login(string? ReturnUrl, LoginVM vm)
    {
        ...
        if (!user.EmailConfirmed)
        {
            ...
            if (utoken?.SendDate.AddHours(24) > DateTime.UtcNow)
            {
                ViewBag.ConfirmationMessage = "Mail already sent. if you dont see in your inbox. Please check spams. You can send " +
                    "Confirmation mail after " + utoken.SendDate.AddHours(24);
            }
            else
            {
                ViewBag.ConfirmationMessage =
                    $"Please confirm your account <a href='/Auth/ConfirmationEmailSent?" +
                    $"username={user.UserName}'>Send Email </a>";
            }
            return View();
        }
        if (!result.Succeeded)
