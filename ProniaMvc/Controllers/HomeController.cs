...
public class HomeController : Controller
{
    ...
    public async Task<IActionResult> AddBasket(int? id)
    {
        ...
        List<BasketItenVM> items = basket == null ? new List<BasketItenVM>() :
            JsonConvert.DeserializeObject<List<BasketItenVM>>(basket);
        var item = items.SingleOrDefault(i => i.Id == id);
        if (item == null)
        {
            item = new BasketItenVM
            {
                Id = (int)id,
                Count = 1
            };
    ...
    {
        var basket = JsonConvert.DeserializeObject<List<BasketItenVM>>(HttpContext.Request.Cookies["basket"]);
